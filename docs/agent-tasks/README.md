...
| [TASK-099](TASK-099-selector-prefetch-candidate-replay.md) | `tools/analyze-galaxy-working-set`: replay bounded orientation-aware selection + rotation prefetch | TASK-098 | pending | **Not on the critical path to a new pack** — every candidate is a runtime selection change reading the same v1 tiles. Answers whether a bounded, frustum-margin, best-first frontier can hold 100% perceptible recall with a rotation prefetch contract. STOP is a successful terminal result. |
