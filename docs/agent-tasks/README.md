...
| [TASK-055](TASK-055-diagnostics-sink.md) | `diagnostics` v1: central `reportError` sink + dev overlay + `assertInvariant` | TASK-054 | pending | **PARTIAL — interim sink + dev overlay shipped ahead-of-track** in `apps/web/src/glue/report-error.ts` (`d38c9f8`): `reportError`/`subscribeErrors`/`errorCount`/`__cosmosErrors`. REMAINING when picked up: promote to a framework-agnostic `diagnostics` package + add `assertInvariant`. new leaf pkg; Sentry-free |
