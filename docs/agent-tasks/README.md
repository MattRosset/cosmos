...
| [TASK-084](TASK-084-system-scene-context-units.md) | `apps/web` + `render-planets`: make SystemScene's bodies context-scaled (206,266× oversized outside `system`) | TASK-081 | pending | **STUB, not spec-reviewed** — filed from `docs/research/m3-switch-delta-yardstick.md` (CLAIM 5). Same contract violation as TASK-081 on the mesh renderers: geometry is baked in AU (`planet-mesh.ts:42`) while offsets arrive in active-context units (`SystemScene.tsx:333`), so in galaxy context Sol draws as an 11°-wide sphere. **Latent in production** — `StarApp` mounts SystemScene only in `system` context; it was the M3 probe app that reached it, which is what turned the m3 switch-invisibility gate red. Photographed, not just derived. |
