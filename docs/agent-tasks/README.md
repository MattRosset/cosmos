...
| [TASK-083](TASK-083-pick-ray-context-units.md) | `apps/web`: convert the star pick ray's origin to parsecs (the pick path's half of TASK-081) | TASK-081 | pending | **STUB, not spec-reviewed** — filed from TASK-081's Out-of-scope instruction. Same contract violation as TASK-081 on a different call chain: `StarScene.tsx:239` passes `controller.state.position.local` (context units) into a pick path documented as tile-local parsecs (`pick.ts:12`), off by 206,266x in `system`. Verified from source, symptom NOT measured. Writeup: `docs/research/star-pick-ray-origin-context-units.md`. A galaxy-context test cannot catch it (`pcScales('galaxy')` is exactly 1) — the gate must pick inside a system, against a star that is actually drawn. |
