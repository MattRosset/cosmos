...
| [TASK-081](TASK-081-point-renderer-context-units.md) | `render-stars` + `render-galaxy`: honor the parsec contract in the point renderers (star field is mispositioned + unlit outside galaxy context) | — | pending | Fixes the measured defect in `docs/research/star-sprite-goes-dark-on-system-entry.md` (luma 255→3, blob 8px→0 in one frame at the galaxy→system flip, point count unchanged). Cause is a contract violation: `setRenderOffset` documents parsecs, all call sites pass active-context units. One uniform, bit-identical in galaxy context. **Spec-reviewed 2026-07-25:** rename nothing (two frozen-text asserts), write the uniform every frame (mounts are created dynamically), clip planes are out of scope but MUST be measured and reported — Sol survives the far-plane clip while the field behind it does not, which is a false-green trap. |
| [TASK-082](TASK-082-impostor-context-scale.md) | `render-galaxy` + `apps/web`: make the galaxy impostor's radius context-invariant (1e6 oversize in universe context) | — | pending | Pre-existing bug, already rendered by `flythrough3`/`m3`/`soak3`: the impostor bakes `mesh.scale` from a parsec radius while its offset arrives in active-context units. Blocks TASK-080 — an ascent must not land on a broken view. |
