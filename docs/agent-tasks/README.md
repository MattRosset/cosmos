...
| [TASK-056](TASK-056-app-boundary-sentry.md) | `apps/web`: ErrorBoundary (DOM+scene) + global handlers | TASK-055 | done | Scope closed to what shipped in `d38c9f8`: ErrorBoundary (DOM+scene) + global handlers + WebGL2 guard — white screen killed, verified live. **Split 2026-07-22:** the Sentry transport never shipped (no `@sentry/react`, no `VITE_SENTRY_DSN`) and now lives in TASK-069. One row cannot hold two states — this one was reading `pending` while TASK-059, which depends on the shipped half, read `done`. |
...
| [TASK-069](TASK-069-sentry-transport.md) | `apps/web`: Sentry transport behind the error sink | TASK-055 | pending | Split out of TASK-056 on 2026-07-22 (that row bundled three deliverables; two shipped, this one did not). Adds `@sentry/react` + `VITE_SENTRY_DSN`-gated transport; source-map upload belongs in the CF Pages build, not `.github/workflows/` (the Actions deploy workflow was removed). |
