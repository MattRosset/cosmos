...
| [TASK-080](TASK-080-universe-ascent.md) | `nav` + `apps/web`: unblock the galaxy→universe ascent (lift `ownGalaxyContext`) + `◂ Universe` breadcrumb | TASK-082 | pending | Item 1 of `docs/research/universe-scale-tour-preflight.md`'s ordered list — the only measured **blocker** on the universe-scale tour path. Today `controller.ts:800` gates the exit on a flag production never sets, so `universe` is unreachable while the HUD ruler already advertises it. Ships the gate lift (guarded by "a galaxy anchor is armed", which leaves the TASK-027 control test untouched) **and** the affordance, because free flight to the 100 kpc gate is ~8.5 min of held Shift+W. The galaxy-point field (N2) is the NEXT task, not this one. **Spec-reviewed 2026-07-25:** the return leg is `◂ Galaxy`, not `◂ Milky Way` (`arrivalDistanceM` is a radius, so that vantage lands OUTSIDE the enter gate from universe); blocked on TASK-082. |
