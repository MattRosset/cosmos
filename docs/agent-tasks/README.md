...
| [TASK-084](TASK-084-system-scene-context-units.md) | `apps/web` + `render-planets`: make SystemScene's bodies context-scaled (206,266× oversized outside `system`) | TASK-081 | in-progress | **Implemented 2026-07-27 on `feat/task-084-system-scene-context-units`; awaiting CI.** Added `systemToContextScale` (`apps/web/src/glue/context-scale.ts`, exact `1` in `system`, mirrors `pcScales`'s shape per D1) and applied it per-frame to `mesh.object.scale` / `line.object.scale` / `atm.object.scale` in `SystemScene.tsx` (D2) — never `rootGroup.scale`. Measured end-to-end via a new e2e gate, `e2e/tests/system-context-scale.spec.ts`: reads the LIVE `mesh.object.scale.x` off the THREE scene graph (new `__cosmos.systemBody` hook) against the independent `CONTEXT_UNIT_METERS` formula, and the projected diameter via two real `__cosmos.projectToScreen` calls, run from `?debug=m4a` (the one probe app that already keeps the system mounted off-context). Mutation-tested: reverting the `object.scale` write reproduces a measured **206,265.3×** error (vs. the spec's stated 206,266×) before failing the gate. `m3`'s `enterSystemDelta` stayed at `0.001`, confirming `M3App` is untouched. Notes: `NOTES-2026-07-27-task-084.md`. |
