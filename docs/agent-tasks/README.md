...
| [TASK-040](TASK-040-m3-integration.md) | M3 integration: continuous Milky Way → Sol → Earth zoom, no loading screens | TASK-032–039 (all) | done | exclusive in `apps/web`/`e2e`; composition only; shipped on `task-040-galaxy-view` (galaxy view + streaming tier + `m3.spec.ts` green) |
