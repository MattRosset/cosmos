...
| [TASK-034](TASK-034-pack-octree.md) | `tools/pack-octree`: catalog → Morton-keyed octree tiles + manifest | TASK-031 | done | status synced retroactively (TASK-064); lane shipped within Phase 3 — see TASK-040/041 closure |
| [TASK-035](TASK-035-data-v3.md) | `data` v3: octree manifest + on-demand tile loader (worker decode) | TASK-031, TASK-032 | done | status synced retroactively (TASK-064); lane shipped within Phase 3 — see TASK-040/041 closure |
| [TASK-036](TASK-036-render-galaxy.md) | `render-galaxy` v1: particle clouds + dust lanes + far-LOD impostor | TASK-031 | done | status synced retroactively (TASK-064); lane shipped within Phase 3 — see TASK-040/041 closure |
| [TASK-037](TASK-037-universe-context.md) | `nav` v4: universe⇄galaxy switch + local group of procedural galaxies | TASK-031 | done | status synced retroactively (TASK-064); lane shipped within Phase 3 — see TASK-040/041 closure |
...
| [TASK-039](TASK-039-quality-tiers.md) | `scene-host` v1.2: PerformanceMonitor-driven adaptive quality tiers | TASK-031 | done | status synced retroactively (TASK-064); lane shipped within Phase 3 — see TASK-040/041 closure |
...
| [TASK-056](TASK-056-app-boundary-sentry.md) | `apps/web`: ErrorBoundary (DOM+scene) + global handlers | TASK-055 | done | Scope closed to what shipped in `d38c9f8`: ErrorBoundary (DOM+scene) + global handlers + WebGL2 guard — white screen killed, verified live. **Split 2026-07-22:** the Sentry transport never shipped (no `@sentry/react`, no `VITE_SENTRY_DSN`) and now lives in TASK-078. One row cannot hold two states — this one was reading `pending` while TASK-059, which depends on the shipped half, read `done`. |
...
| [TASK-061](TASK-061-app-tsx-decomposition.md) | Decompose `App.tsx` into per-composition modules (mechanical move) | TASK-053, TASK-060 | done | PR #6. `App.tsx` → 28-line dispatcher; compositions moved verbatim to `apps/web/src/app/` + HUD leaves to `apps/web/src/hud/`. Guided tour stays inlined in `StarApp` (654 lines); per-file cap corrected 620→660 (verbatim size; no shared-hook extraction). `pnpm verify` green; e2e gate is the behavioral proof. |
| [TASK-062](TASK-062-coverage-gate-wiring.md) | Wire dormant coverage gates (core-types + pack tools) + CI pack-octree dedupe | TASK-053 | done | Acceptance re-run 2026-07-22: `pnpm verify` 23/23 exit 0; core-types prints coverage (97.02%) and its gate is live (forcing statements=99 fails); 4 tools wired + green; ci.yml has exactly one `pack-octree test` step. **`pack-solar` is intentionally NOT wired** — 57.7% is under Deliverable 4's floor of 60, so per that rule it stays on plain `vitest run` (see this task's Notes); needs a `cli.ts` test pass in a future task. Acceptance #3 ("each of the five") was written without the Deliverable 4 escape hatch — the deliverable governs. |
| [TASK-063](TASK-063-screenshot-policy-alignment.md) | Screenshot policy alignment: guard 3 CI-blocking `toHaveScreenshot` + e2e README | TASK-053 | done | **Merged (PR #8, `56bb75f`); status synced 2026-07-27** (audit had confirmed it shipped). Maintenance track; `e2e/` only; testing-conventions §1.4 wins over e2e/README taxonomy. **STATUS AUDIT 2026-07-22: code appears shipped — all 8 `toHaveScreenshot` call sites are guarded (6 via `if (!process.env['CI'])`, `perception-cards`/`perception-scale` via describe-level `test.skip`), and `e2e/README.md:17` already states reference-machine-only. Left `pending` pending an acceptance run; verify and close rather than re-implement.** |
| [TASK-064](TASK-064-docs-truth-sync.md) | Docs truth sync: architecture §4/§12/§13, stale status rows, core-types README | TASK-053, TASK-062, TASK-063 | done | Maintenance track; docs only; records approved doctrine (no fast-check/SSIM/LFS/pinned runner) |
| [TASK-065](TASK-065-gaia-manifest-env-config.md) | Env-configurable Gaia octree manifest URL (production-pack readiness) | TASK-053, TASK-061 | done | Shipped: `apps/web/src/app/packs.ts` reads `VITE_GAIA_OCTREE_MANIFEST_URL` (build-time, Vite-inlined), default = the committed 135-star sample; declared in `apps/web/src/vite-env.d.ts`. Status corrected 2026-07-22 |
| [TASK-066](TASK-066-ui-perception-literacy.md) | `ui` perception v1 — literacy: human units, mode badge, hints | TASK-053, TASK-061 | done | Shipped: `ui/src/ModeBadge.tsx` + human-unit formatting, live in the demo. Status corrected 2026-07-22. Perception track (research `ui-ux-perception-and-polish.md` Phase 1); sanctioned additive `ui` v4 thaw; zero nav changes; serialize with TASK-065 (`apps/web`) |
| [TASK-067](TASK-067-ui-scale-perception.md) | `ui` perception v2 — scale ruler + unified Jump HUD + letterbox kit | TASK-066 | done | Shipped: `ui/src/ScaleRuler.tsx` + `JumpHud.tsx` (+ `scale-ruler.ts`/`jump-hud-model.ts` pure models), live in the demo. Status corrected 2026-07-22. Perception track Phase 2; letterbox+copy only (no post-pass — integrated-GPU floor); exclusive in `apps/web` |
| [TASK-068](TASK-068-ui-cards-identity.md) | `ui` perception v3 — insight cards + visual identity | TASK-066, TASK-067 | done | Shipped: `ui/src/InfoPanel.tsx` cards + `ViewDrawer.tsx` + visual identity, live in the demo (PR #18). Status corrected 2026-07-22. Perception track Phase 3; display-time derivation only, no new data pipeline; C3 card-only (search badges wait on Gaia/search lane) |
| [TASK-078](TASK-078-sentry-transport.md) | `apps/web`: Sentry transport behind the error sink | TASK-055 | pending | Split out of TASK-056 on 2026-07-22 (that row bundled three deliverables; two shipped, this one did not). Adds `@sentry/react` + `VITE_SENTRY_DSN`-gated transport; source-map upload belongs in the CF Pages build, not `.github/workflows/` (the Actions deploy workflow was removed). |
| [TASK-079](TASK-079-octree-flush-during-flight.md) | `apps/web`: flush deferred octree tiles DURING flight (fix black galaxy→star-field fly-in on Metal) | — | done | Root-caused + researched (`docs/research/galaxy-starfield-flyin-black-flush-during-flight.md`, verdict ENABLE). Drain runs during flight at `OCTREE_FLUSH_PER_FRAME_FLYING=8`, keeping the post-arrival 2/frame. Visibility fix verified live (fly-in `lumaMax` 15→765 at matched `outLoaded≈204`). `CI=true pnpm test:e2e` = 39 pass / 1 fail, the fail being `boot-perf` (single boot frame ~3.1 s on this M1) which fails IDENTICALLY on `main` (git-checkout control) — pre-existing reference-machine issue, not this change. The intermittent ~65 ms fly-in frame is likewise pre-existing (git-stash control); see `NOTES-2026-07-23-task-079.md`. **Root-caused 2026-07-24** (`docs/research/m1-metal-boot-and-flyin-stall-rootcause.md`): the boot-perf ~3.1 s frame is `getContext('webgl2')` under the gate's **SwiftShader** backend (3 ms on real Metal — a test-measurement artifact, not an app/Metal bug); the ~65 ms fly-in frame is `streaming.update` **BigInt Morton** (fix = BigInt→`Number` swap, ~65.9×). The two are unrelated. |
| [TASK-080](TASK-080-universe-ascent.md) | `nav` + `apps/web`: unblock the galaxy→universe ascent (lift `ownGalaxyContext`) + `◂ Universe` breadcrumb | — | done | **Merged (`88f829e`); status synced 2026-07-27** — the ascent gate (`ownGalaxyContext || galAnch !== null`) is live at `controller.ts:797` and the `◂ Universe` breadcrumb shipped. Item 1 of `docs/research/universe-scale-tour-preflight.md`'s ordered list — the only measured **blocker** on the universe-scale tour path. Today `controller.ts:800` gates the exit on a flag production never sets, so `universe` is unreachable while the HUD ruler already advertises it. Ships the gate lift (guarded by "a galaxy anchor is armed", which leaves the TASK-027 control test untouched) **and** the affordance, because free flight to the 100 kpc gate is ~8.5 min of held Shift+W. The galaxy-point field (N2) is the NEXT task, not this one. **Spec-reviewed 2026-07-25:** the return leg is `◂ Galaxy`, not `◂ Milky Way` (`arrivalDistanceM` is a radius, so that vantage lands OUTSIDE the enter gate from universe); the TASK-082 dependency was REMOVED by the 2026-07-26 spec review (its premise was measured false — see Decision 2 and docs/research/galaxy-impostor-scale-is-inert.md); at this task’s own 0.18 Mpc arrival vantage the impostor owes 0% of the galaxy’s brightness on any canvas ≥900 px tall, so it is not blocked. |
| [TASK-081](TASK-081-point-renderer-context-units.md) | `render-stars` + `render-galaxy`: honor the parsec contract in the point renderers (star field is mispositioned + unlit outside galaxy context) | — | done | Merged 2026-07-26 (`d3e4fa5`, PR #28); `pcScales` now lives at `apps/web/src/glue/context-scale.ts`. Fixes the measured defect in `docs/research/star-sprite-goes-dark-on-system-entry.md` (luma 255→3, blob 8px→0 in one frame at the galaxy→system flip, point count unchanged). Cause is a contract violation: `setRenderOffset` documents parsecs, all call sites pass active-context units. One uniform, bit-identical in galaxy context. **Spec-reviewed 2026-07-25:** rename nothing (two frozen-text asserts), write the uniform every frame (mounts are created dynamically), clip planes are out of scope but MUST be measured and reported — Sol survives the far-plane clip while the field behind it does not, which is a false-green trap. |
| [TASK-082](TASK-082-impostor-context-scale.md) | `render-galaxy` + `apps/web`: make the galaxy impostor render at all, at a context-invariant size | TASK-081 | done | **Merged (PR #30, `1710134`); status synced 2026-07-27.** The radius now travels as a uniform and is applied in the vertex shader, sourced from `discRadiusPc` (D2). Pixel gate is `e2e/tests/universe-impostor-scale.spec.ts` — vitest has no WebGL in this workspace, so D4 moved to e2e as TASK-085 did (NOTES JC-1). Measured: the impostor now contributes 21% of the frame's lit pixels at the 0.18 Mpc vantage, and **exactly 0** pre-change. The spec's OPEN question is answered — the context scale is visually **inert** (bit-identical frames), kept for the depth/clip budget only, a deliberate deviation logged as JC-4. Notes: `NOTES-2026-07-26-task-082.md`. **Respecified 2026-07-26 — the original premise was false.** It was filed as "the impostor is 1e6× oversized in universe"; measured, `radiusUnits` reaches **no pixel in any context** (the vertex shader references neither `modelMatrix` nor `modelViewMatrix`, so `mesh.scale` is inert), and the quad is a sub-pixel 1-unit plane that has never drawn in production. The old `mesh.scale` unit test is what hid it, and the original deliverable-3 test would also have passed without moving a pixel. Measurements: `docs/research/galaxy-impostor-scale-is-inert.md`. Blast radius is universe context only — provably 0 opacity in galaxy context below the 100,009.7 pc exit gate at any viewport ≥ 493 px — but it owes **84%** of the galaxy's brightness at `flythrough3`'s 0.6 Mpc start. Size S→**M**. Also found: the fed radius is a **random draw** (28,002.33 pc), not the "≈15,000" the old spec asserted, and disagrees 1.87× with streaming's `discRadiusPc`. Blocks TASK-080. |
| [TASK-083](TASK-083-pick-ray-context-units.md) | `apps/web`: convert the star pick ray's origin to parsecs (the pick path's half of TASK-081) | TASK-081 | done | **Implemented 2026-07-27 on `feat/task-083-pick-ray-context-units`.** Scaled the star-pick ray origin by `pcScales(controller.contextId).unitsToPc` at the single site (`StarScene.tsx:~260`), leaving the two other `controller.state.position.local` reads (galaxy pick `:235`, `projectToScreen` `:278`) untouched — both are self-consistent in their own frames (spec §Frozen surface, verified by measurement). Symptom measured live first (readiness gate): via `?debug=m4a` in settled `system` context, 8,125/109,399 HYG stars project on-screen and `pickAt` returned a WRONG id at every one (0/40 match); `pcToUnits(system)=206266.30`. New gate `e2e/tests/system-context-pick.spec.ts` projects real catalog stars through `__cosmos.projectToScreen` (parsecs→system units via `__cosmos.contextUnitMeters`, no hardcoded ratio/pixel) and asserts `pickAt` at the projected pixel of an isolated on-screen star resolves THAT star. Power-proven: **FAILs on `main`** (target `hyg:16496` → `pickAt` `hyg:16729`), **PASSes with fix** (→ `hyg:16496`), same deterministic target. Notes: `NOTES-2026-07-27-task-083.md`. |
| [TASK-084](TASK-084-system-scene-context-units.md) | `apps/web` + `render-planets`: make SystemScene's bodies context-scaled (206,266× oversized outside `system`) | TASK-081 | done | **Merged (PR #34, `12f6d95`); status synced 2026-07-27.** Added `systemToContextScale` (`apps/web/src/glue/context-scale.ts`, exact `1` in `system`, mirrors `pcScales`'s shape per D1) and applied it per-frame to `mesh.object.scale` / `line.object.scale` / `atm.object.scale` in `SystemScene.tsx` (D2) — never `rootGroup.scale`. Measured end-to-end via a new e2e gate, `e2e/tests/system-context-scale.spec.ts`: reads the LIVE `mesh.object.scale.x` off the THREE scene graph (new `__cosmos.systemBody` hook) against the independent `CONTEXT_UNIT_METERS` formula, and the projected diameter via two real `__cosmos.projectToScreen` calls, run from `?debug=m4a` (the one probe app that already keeps the system mounted off-context). Mutation-tested: reverting the `object.scale` write reproduces a measured **206,265.3×** error (vs. the spec's stated 206,266×) before failing the gate. `m3`'s `enterSystemDelta` stayed at `0.001`, confirming `M3App` is untouched. Notes: `NOTES-2026-07-27-task-084.md`. |
| [TASK-085](TASK-085-overlay-context-scale.md) | `render-fx` + `apps/web` + `e2e`: give the overlay nebulae + constellation line-set a context scale (the universe vantage washes to near-white) | TASK-081 | done | **Spec-reviewed 2026-07-26; corrects a falsified attribution.** `docs/research/universe-vantage-nebula-blowout.md` blamed the procgen dust lanes; ablated since, they contribute **0.004%** of the effect and are angularly correct (their offset AND geometry are both parsecs, so the 1e6× error is uniform and a perspective projection is invariant to it). The wash is `Overlays.tsx`, the last per-frame site handing a renderer a raw `toRenderSpace` result: correct context-unit offset + parsec geometry = **non-uniform**, 1e6× oversized in `universe`. Measured at the settled 0.18 Mpc vantage: nebula alone = mean luma 250.58 / 98.1% of pixels above luma 200; with nebula + line-set removed the whole galaxy collapses to one 60×58 px object. Independently confirmed by flipping the quality tier (`nebulaeAllowed = tier !== 'low'`) with nothing else changed — which is also why the gate MUST pin the tier or pass vacuously. Measurements: `docs/research/universe-vantage-blowout-is-the-overlays.md`. Blocks TASK-080. |
| [TASK-086](TASK-086-local-group-galaxy-field.md) | `nav` + `apps/web`: render + make selectable the 11 local-group galaxies in `universe` context (N2) | — | done | Renders `makeLocalGroup()`'s indices 1..N-1 (index 0 is a vestigial seed-donor slot, not the origin Milky Way) as `createGalaxyImpostor` sprites via a new sibling `LocalGroupScene`, additive to `GalaxyScene` (untouched). Click-pick added to `StarScene.pickAt` as a universe-context-only branch (owns the click fully, ahead of the planet/star fallback) via a new pure `pickNearestGalaxy` (`packages/nav`), mirroring `pickNearestStar`'s angular-nearest shape; deterministic names via a new pure `localGroupGalaxyName` (`proc:localgroup:<n>` → `Galaxy G-<n>`, `proc:milkyway` → `Milky Way`), wired into `Breadcrumb.tsx`'s one fallback line. `__cosmosDev.setLocalGroupVisible` (ablation, mirrors `setImpostorRadiusPc`) and `__cosmosDev.orientTo` (facing-only reorientation via a negligible-distance `goTo`, since `goTo` no-ops on a zero-distance target) back the e2e gates. Blocked-by omitted here (not "—" pointing at TASK-080's own stale `pending` row) though the ascent it renders into has been live on `main` since TASK-080/081/082 merged — see NOTES JC-5. |
| [TASK-096](TASK-096-visibility-mode-contract.md) | `docs`: freeze the Natural/Survey star-visibility contract (ADR-007) | — | done | ADR-007 written 2026-08-06. Natural = today's enriched default (slider 25, octree ×6 ⇒ effective 150); Survey = exposure-only depth (×40 ⇒ 1000), never called Telescope; one floor `0.004` shared by render/pick/cull/streaming. Records the trade in decision 9: Survey lifts the faint tail over the same floor, so photometric streaming reductions shrink there — Natural is the default and is where the saving lives. Motivated by the measured 7.90% perceptible share near Sol (`../research/galaxy-octree-streaming-value-near-sol.md` Claim 5). |
| [TASK-097](TASK-097-shared-star-photometry.md) | `packages/photometry` (new) + `render-stars` + `apps/web`: extract the shared CPU photometry contract | TASK-096 | pending | The apparent-magnitude/point-size/perceptibility math exists twice today — GLSL, and a hand-copied CPU replay in `tile-brightness-cull.ts` — plus duplicated constants (8/3/64, 0.004, ×6). Behavior-frozen extraction, gated by a regression fixture that must report zero changed booleans and an empty shader diff. On the critical path whatever the streaming verdicts turn out to be. |
| [TASK-098](TASK-098-co-timed-streaming-diagnostics.md) | `streaming` + `apps/web`: one co-timed `galaxyWorkingSet()` snapshot | TASK-097 | pending | Replaces comparisons between unrelated ≤4 Hz mirrors with a single synchronous query stamped by the same policy update sequence. Not required by TASK-101 (the existing `streaming.{loadedChunks,cutSize}` hooks suffice for parity), so it does not gate re-packing. |
| [TASK-099](TASK-099-selector-prefetch-candidate-replay.md) | `tools/analyze-galaxy-working-set`: replay bounded orientation-aware selection + rotation prefetch | TASK-098 | pending | Runtime only — every candidate changes what the selector fetches, never what the pack stores, so it neither gates nor is gated by re-packing. Answers whether a bounded, frustum-margin, best-first frontier can hold 100% perceptible recall with a rotation prefetch contract. STOP is a successful terminal result. |
| [TASK-100](TASK-100-pick-perceptibility-gate.md) | `apps/web`: stop the Gaia pick from claiming imperceptible stars | TASK-097 | pending | Measured: `octree-pick.ts` scans every point in every drawn tile with no brightness test, and only 7.90% of them reach the floor — clicking empty sky opens the card of a star that draws no pixel. Power test must fail before the fix. Independent of TASK-098/099. |
| [TASK-101](TASK-101-photometric-band-layout-replay.md) | `tools/analyze-galaxy-working-set`: replay pack-FORMAT candidates (P0-P3) | TASK-097 | pending | The pack-FORMAT question, independent of TASK-099 (runtime) — briefly scheduled as the critical path when re-packing was to precede hosting, an ordering dropped the same day. Subtree `minAbsMag` (manifest-only) plus magnitude-band and bright-prefix layouts (tile binaries), measured per profile against production photometry. A STOP is a good outcome: v1 stands and the existing pack ships as is. |
| [TASK-103](TASK-103-pick-perceptibility-gate-hyg-exo.md) | `apps/web`: stop the HYG/exo star pick from claiming imperceptible stars | TASK-097 | in-progress | VIS-06b — TASK-100 applied to the sibling pick path. `pickNearestStar` calls `pickStar` with geometry only, so clicking dark sky selects a sub-floor HYG star that draws no pixel. Gate each candidate via `starIsPerceptible` in a new pure app-glue `pickNearestVisibleStar` (mirrors `octree-pick.ts`; `render-stars`/`pick.ts` stay frozen — Decision (b)). Power test must fail before the fix. Independent of TASK-102 (HYG/exo multiplier = 1 in both modes, so mode is inert here — thread it anyway for one source of truth). |
| [TASK-104](TASK-104-post-chain-foundation.md) | `scene-host`: post-processing chain foundation (tier-gated identity composer) | TASK-039, TASK-047 | done | Render-quality lane step 1/3 (foundation → AA TASK-105 → bloom TASK-106). Mounts a `bloomEnabled`-gated identity `<EffectComposer>` — no visible effect yet. **Step-0 blocker RESOLVED** (`../research/post-chain-probe-render-conflict.md`): the composer-vs-probe `gl.render` collision does NOT move budget gates (high-tier gates read policy CPU stats; the one `gl.info` gate is low-tier) — fix is an additive `SceneHostProps.postProcessing` flag the 6 manual-render probe apps set false (owner-resolved 2026-09-07). One genuine precondition the executor verifies first (Step 0.5): a `@react-three/postprocessing` version must resolve cleanly against R3F 9 / three 0.184 / React 19 — if none does, STOP (do not force-install). **DONE 2026-09-07:** resolved on `@react-three/postprocessing@3.0.4` (peer R3F `^9.0.0`, clean install, no force/overrides). Identity required an 8-bit composer target (`frameBufferType=UnsignedByteType`) so additive star sprites clamp like the direct path — HalfFloat default was NOT a no-op (`../research/post-chain-identity-hdr-target.md`; TASK-106 restores HDR for bloom). Gates green: unit `post-chain.test.tsx` + `pnpm verify` + deterministic e2e (smoke/error-gate/m3 pass; no threshold relaxed). |
