...
| [TASK-102](TASK-102-survey-visibility-mode.md) | `app-state` + `ui` + `apps/web`: wire the Natural/Survey visibility mode (state + toggle + consumers) | TASK-096, TASK-097 | pending | VIS-05 — the mode-state task every Stage-0 seam (TASK-097 render/cull, TASK-100 pick) was written against. Mechanical wiring: add `mode` to `useSettingsStore` (sanctioned additive app-state thaw), a Survey toggle in `ViewDrawer`, and change the three hard-selected `NATURAL_VISIBILITY_PROFILE` sites to read `VISIBILITY_PROFILES[mode]`. Exposure-only per ADR-007 §3 (never "Telescope", no FOV/reticle, no persistence). Gate is deterministic + pack-agnostic (applied effective-exposure holder 150→1000 + runtime-selected survey-only pick), power/mutation-proven. |
