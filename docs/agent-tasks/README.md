...
| [TASK-062](TASK-062-coverage-gate-wiring.md) | Wire dormant coverage gates (core-types + pack tools) + CI pack-octree dedupe | TASK-053 | pending | Maintenance track; scripts/config/ci.yml only, no test or source edits. **STATUS AUDIT 2026-07-22: code appears shipped — 23 `vitest.config.ts` carry `thresholds` (every package + all 5 pack tools) and `ci.yml:88` records the pack-octree dedupe. Left `pending` because the acceptance check was never run; verify and close rather than re-implement.** |
